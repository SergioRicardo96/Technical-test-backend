...
<?php startSection('content'); ?></h1>
    ...
    <form method="POST" action="/admin/tasks">
        @csrf
        <h1 class="h3 mb-4 fw-normal"><?= trans('tasks', 'create_task') ?></h1>
        ...
        <div class="form-floating mb-3">
            <select class="form-select" id="floatingSelectTaskStatus" aria-label="Floating label select example" name="catalog_task_status_id">
                ...
                <?php foreach ($taskStatuses as $taskStatus): ?>
                    <option value="<?= $taskStatus['id'] ?>" <?= (isset($data['catalog_task_status_id']) && $data['catalog_task_status_id'] == $taskStatus['id']) ? 'selected' : '' ?>>
                        <?= $taskStatus['name'] ?>
                    </option>
                <?php endforeach ?>
            </select>
        ...
        <div class="form-floating mb-3">
            <input type="text" class="form-control" id="titleInput" name="title" placeholder="<?= trans('tasks', 'title') ?>" value="<?= $data['title'] ?? ''; ?>">
        ...
        <div class="form-floating">
            <textarea type="text" class="form-control" id="floatingDescription" name="description" placeholder="<?= trans('tasks', 'description') ?>" style="height: 100px"><?= $data['description'] ?? ''; ?></textarea>
        ...
        <button class="w-100 btn btn-lg btn-primary mt-4" type="submit"><?= trans('tasks', 'create_task') ?></button>
    </form>
